...
export const authConfig = {
  ...
  rateLimit: {
    ...
    storage: "database" as const, // Store rate limit data in database
  },
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async (
      ...
      await Logger.info(
        `Sending password reset email to user: ${user.email}, name: ${user.name}`,
        { type: "auth" }
      );
    ...
    onPasswordReset: async ({ user }: { user: User }, _request?: Request) => {
  ...
  onSignIn: async ({ user }: { user: User }, request?: Request) => {
    await Logger.info(`User signed in: ${user.email}, name: ${user.name}`, {
      type: "auth",
      userId: user.id,
    });
  },
  onEmailVerification: async ({ user }: { user: User }, request?: Request) => {
    await Logger.info(
      `User verified email: ${user.email}, name: ${user.name}`,
      { type: "auth", userId: user.id }
    );
  },
};
